...
public class InvertedIndexMovieSearchService implements MovieSearchService {
	...
	public void init() {
		for (Movie movie : movieRepository.findAll()) {
			String[] words = movie.getName().toLowerCase().split("\\W+");
			for (String word : words) {
				invertedIndex.computeIfAbsent(word, k -> new HashSet<>()).add(movie.getId());
			}
		}
	}
	@Override
	public List<Movie> search(String queryText) {
		...
		String[] queryWords = queryText.toLowerCase().split("\\W+");
		Set<Long> resultIds = new HashSet<>(invertedIndex.getOrDefault(queryWords[0], Collections.emptySet()));

		for (String word : queryWords) {
			resultIds.retainAll(invertedIndex.getOrDefault(word, Collections.emptySet()));
		}
